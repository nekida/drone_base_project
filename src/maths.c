...
float get_cos_approx (float x)
{
    return get_sin_approx(x + (0.5f * M_PIf));
}
...
float get_atan2_approx (float y, float x)
{
    ...
    res = (res) ? (MIN(absX, absY) / res) : (0.0f);
...
{
    return (!denom) ? num : gcd(denom, num % denom);
}

int32_t get_wrap_18000 (int32_t angle)
{
    if (angle > 18000)
        angle -= 36000;
    if (angle < -18000)
...
int32_t get_wrap_36000 (int32_t angle)
{
    if (angle > 36000)
        angle -= 36000;
    if (angle < 0)
...
int32_t apply_deadband_rescaled (int32_t value, int32_t deadband, int32_t min, int32_t max)
{
    if (ABS(value) < deadband) {
        value = 0;
    } else if (value > 0) {
...
void devClear (stdev_ts *dev)
{
    dev->m_n = 0;
}

void devPush (stdev_ts *dev, float x)
{
    dev->m_n++;
    if (dev->m_n == 1) {
...
float devVariance (stdev_ts *dev)
{
    return ((dev->m_n > 1) ? dev->m_newS / (dev->m_n - 1) : 0.0f);
}

float devStandardDeviation (stdev_ts *dev)
{
...
void rotationMatrixFromAngles (fp_mat3_ts *rmat, const fp_angles_tu *angles)
{
    float cosx, sinx, cosy, siny, cosz, sinz;
    float coszcosx, sinzcosx, coszsinx, sinzsinx;

    cosx = get_cos_approx(angles->angles.roll);
    sinx = get_sin_approx(angles->angles.roll);
    cosy = get_cos_approx(angles->angles.pitch);
    siny = get_sin_approx(angles->angles.pitch);
    cosz = get_cos_approx(angles->angles.yaw);
    sinz = get_sin_approx(angles->angles.yaw);
...
void rotationMatrixFromAxisAngle(fp_mat3_ts *rmat, const fp_axis_angle_ts *a)
{
    const float sang = get_sin_approx(a->angle);
    const float cang = get_cos_approx(a->angle);
 ...
 */
void sensorCalibrationResetState(sensorCalibrationState_ts * state)
{
    for (int i = 0; i < 4; i++){
        for (int j = 0; j < 4; j++){
            state->XtX[i][j] = 0;
...
void sensorCalibrationPushSampleForOffsetCalculation(sensorCalibrationState_ts *state, int32_t sample[3])
{
...
void sensorCalibrationPushSampleForScaleCalculation(sensorCalibrationState_ts * state, int axis, int32_t sample[3], int target)
{
    for (int i = 0; i < 3; i++) {
...
bool sensorCalibrationSolveForOffset(sensorCalibrationState_ts * state, float result[3])
{
    float beta[4];
...
bool sensorCalibrationSolveForScale(sensorCalibrationState_ts * state, float result[3])
{
    float beta[4];
    sensorCalibration_SolveLGS(state->XtX, beta, state->XtY);
