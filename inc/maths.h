...
typedef struct {
    float value;
    uint32_t timestamp;
} filterWithBufferSample_ts;

typedef struct {
    uint16_t filter_size;
    uint16_t sample_index;
    filterWithBufferSample_ts * samples;
} filterWithBufferState_ts;

typedef struct {
    float XtY[4];
    float XtX[4][4];
} sensorCalibrationState_ts;
...
#define tan_approx(x)       (get_sin_approx(x) / get_cos_approx(x))
#define asin_approx(x)      (M_PIf / 2 - get_acos_approx(x))
#else
